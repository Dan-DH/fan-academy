...
export class RematchButton extends Phaser.GameObjects.Container {
  ...
  constructor(context: GameScene) {
    ...
    this.buttonImage = context.add.image(0, 0, 'gameAtlas', 'popupButton').setOrigin(0.5).setScale(1.2).setInteractive({ useHandCursor: true });
    ...
    this.buttonImage.on('pointerdown', async () => {
      new ChallengePopup({
        ...
        username: context.playerData!.find( player => player._id === context.opponentId!)?.username
      });
      return;
    });
