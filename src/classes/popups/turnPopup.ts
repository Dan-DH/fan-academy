...
export class TurnWarningPopup extends Phaser.GameObjects.Container {
  ...
  constructor(context: GameScene) {
    ...
    this.okButtonImage = context.add.image(-90, 60, 'gameAtlas', 'popupButton').setTint(0x007BFF).setDisplaySize(110, 60).setInteractive({ useHandCursor: true });
    this.cancelButtonImage = context.add.image(90, 60, 'gameAtlas', 'popupButton').setTint(0x990000).setDisplaySize(110, 60).setInteractive({ useHandCursor: true });
