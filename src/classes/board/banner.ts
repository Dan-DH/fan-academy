...
export class Banner extends Phaser.GameObjects.Container {
  ...
  constructor(context: GameScene, board: Board, playerData: IUserData[]) {
    ...
    this.playerOneBanner = context.add.image(0, 0, 'gameAtlas', 'player_banner').setOrigin(0.5).setPosition(705, 80).setFlipX(true).setTint(0x3399ff);
    this.playerTwoBanner = context.add.image(0, 0, 'gameAtlas', 'player_banner').setOrigin(0.5).setPosition(1095, 80).setTint(0x990000);
    ...
    this.vsBanner = context.add.image(0, 0, 'gameAtlas', 'vs_banner').setOrigin(0.5).setPosition(900, 75);
